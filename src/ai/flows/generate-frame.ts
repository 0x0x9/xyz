...
{{#if prompt}}
La description textuelle est : {{{prompt}}}
{{/if}}

{{#if photoDataUri}}
Utilisez cette image comme inspiration principale pour la mise en page, les couleurs, le thème et la structure : {{media url=photoDataUri}}
{{/if}}

**Format de sortie attendu (JSON) :**
Votre réponse JSON doit impérativement contenir les clés \`explanation\`, \`htmlCode\`, \`cssCode\`, et \`jsCode\`.
`,
});

const frameFlow = ai.defineFlow(
  {
    name: 'frameFlow',
  ...
  async (input) => {
    ...
    if (!output) {
      throw new Error("(X)frame n'a pas pu générer de maquette. Veuillez reformuler votre description.");
    }
    return output;
  }
);
