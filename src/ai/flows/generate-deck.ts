...
const deckFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    if (!output) {
      throw new Error("(X)deck n'a pas pu générer de présentation. Veuillez reformuler votre sujet.");
    }
    return output;
  }
);
